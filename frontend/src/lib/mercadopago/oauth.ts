...
// redirectUri is passed in (computed from the incoming request's origin)
// rather than read from an env var — keeps this working across
// localhost/preview/production without extra config per environment.
export function getMpOAuthConfig(redirectUri: string): MpOAuthConfig | null {
...
/** PKCE — recommended by Mercado Pago for the authorization code flow. */
...
export function isTokenExpired(expiresAtIso?: string | null, skewMs = 5 * 60_000): boolean {
